...
@echo on
call odin.exe build . -o:speed -target:js_wasm32 -out:web/triangle.wasm -extra-linker-flags:"--export-table --import-memory --initial-memory=%INITIAL_MEMORY_BYTES% --max-memory=%MAX_MEMORY_BYTES%"


@REM for /f "delims=" %%i in ('odin.exe root') do set "ODIN_ROOT=%%i"
@REM copy "%ODIN_ROOT%\vendor\wgpu\wgpu.js" "web\wgpu.js"
@REM copy "%ODIN_ROOT%\core\sys\wasm\js\odin.js" "web\odin.js"
